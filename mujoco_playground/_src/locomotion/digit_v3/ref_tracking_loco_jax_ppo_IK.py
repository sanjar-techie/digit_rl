...
class DigitRefTracking_Loco(digit_base.DigitEnv):
  ...
  def _post_init(self) -> None:
    ...
    self.reference_dataset_path = os.path.join(dir_path, "walking_npz_IK")
    self.ref_loader = ref_loader.JaxReferenceLoader(ref_traj_path=self.reference_dataset_path)
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    info = {
        ...
        # "base_local_ee_pos": jp.ravel(self.base_local_ee_pos),
        "qpos_history": qpos_history,
        "ref_motion_lens": jp.array(len(self.ref_loader.ref_data["ref_base_local_pos"])),
        "ref_base_local_pos": jp.array(self.ref_loader.ref_data["ref_base_local_pos"]),
        "ref_base_local_trans": jp.array(self.ref_loader.ref_data["ref_base_local_trans"]),
        ...
        "ref_motor_joint_vel": jp.array(self.ref_loader.ref_data["ref_qvel"]),
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    ).at[-1].set(jp.ravel(state.info["ref_base_local_ee_pos"][future_index]))
    ...
    # new_state = self._reset_environment(state.info, cmd_rng)

    # jax.debug.print("Keys in state.info:", list(state.info.keys()))
    # state.info = jax.lax.cond(
    #     done | (state.info["step"] > state.info["ref_motion_lens"] - 1),
    ...
    state.info["step"] = jp.where(
        done | (state.info["step"] > state.info["ref_motion_lens"] - 10),
        0,
        state.info["step"],
    )
  ...
  def _get_obs(
    ...
    privileged_obs = jp.concatenate([
        ...
        jp.ravel(self.base_local_ee_pos), 
        info["ref_base_local_trans"][info["step"]], # 3
        # info["ref_base_local_pos"][info["step"]], # 3
        # info["ref_base_local_ori"][info["step"]], # 3
        info["ref_base_robot_lin_vel"][info["step"]], # 3
        info["ref_base_robot_ang_vel"][info["step"]], # 3
        info["ref_actuator_targets"][info["step"]], # 20
        info["ref_motor_joint_vel"][info["step"]], # 20
        jp.ravel(info["ref_base_local_ee_pos"][info["step"]]), # 12
        info["last_act"],  # 20
        self.kp*0.01,
        self.kd*0.01,
  ...
  def _reset_environment(self, info, rng):
    """Fully resets the robot state along with the reference trajectory."""


    # Reinitialize mjx_env state
    new_data = mjx_env.init(
        self.mjx_model, 
        qpos=self._init_q,  # Reset to initial qpos
        qvel=jp.zeros(self.mjx_model.nv)  # Reset velocities to zero
    )
    
    # Base state in initial local coordinate
    self.base_local_pos = jp.dot(self.base_local_rot, new_data.qpos[0:3])
    self.base_local_trans = self.base_local_pos - self.base_local_init_pos
    self.base_local_quat = self.quaternion_multiply(self.quaternion_inverse(self.base_init_quat), new_data.qpos[3:7])
    self.base_local_ori = self.quat2euler(self.base_local_quat, axes='rxyz')
    self.base_local_lin_vel = jp.dot(self.base_local_rot, new_data.qvel[:3])
    self.base_local_ang_vel = jp.dot(self.base_local_rot, new_data.qvel[3:6])

    # base states in robot coordinate
    self.base_robot_rot = self.quat2mat(new_data.qpos[3:7])
    self.base_robot_lin_vel = jp.dot(self.base_robot_rot.T, new_data.qvel[0:3])
    self.base_robot_ang_vel = new_data.qvel[3:6]

    # End effector world/local positio
    self.base_world_ee_pos = new_data.xpos[self.ee_index] - new_data.qpos[:3]
    self.base_local_ee_pos = jp.dot(self.base_local_rot, self.base_world_ee_pos.T).T
    ...
    obs = self._get_obs(new_data, new_info, rng)
    reward, done = jp.zeros(2)

    return mjx_env.State(new_data, obs, reward, done, metrics, new_info)
